...
8. **Next Steps:** If pursuing, what are the immediate action items?`;
  ...
  server.prompt(
    ...
    async ({ opportunityId }) => {
      ...
      // Fetch in parallel: SupplyTiger's publog items, opportunity's publog items, related awards
      const [supplyTigerItems, opportunityItems, relatedAwards] = await Promise.all([
        // What SupplyTiger CAN supply (our PSC codes)
        ...
        // What the opportunity REQUIRES (opportunity's PSC code)
        opportunity.pscCode
          ? prisma.nationalStockNumber.findMany({
              where: { pscCode: opportunity.pscCode },
              select: {
          ...
          : Promise.resolve([]),
        // Historical awards from same buying org for precedent analysis
